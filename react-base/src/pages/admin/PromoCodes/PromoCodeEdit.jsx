...
const PromoCodeEdit = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://filmgo.io.vn/api/promocodes/show/${id}`)
      .then((response) => {
        ...
        form.setFieldsValue({
          ...
          status: data.status === "active", // Chuyển thành boolean
        });
      })
      .catch(() => {
  ...
  const handleUpdate = (values) => {
    ...
    // Chuyển status từ boolean về string "active"/"inactive"
    const updatedData = {
      ...values,
    ...
    axios
      .put(`http://filmgo.io.vn/api/promocodes/update/${id}`, updatedData)
      .then((response) => {
